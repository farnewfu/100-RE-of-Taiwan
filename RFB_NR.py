...
for w in range(5): 
    ...
    while wind_c <=33000:
        ...
        if w < 7 :
            while  math.ceil(x*1000)/1000.0 != 0.60 :
                ...
                while e1[8759] <= e1[0]:
                    ...
                    for i in range(365): #計算e1(MWh)
                            for j in range(24):
                                if i == 0 and j == 0 :
                                    e1[0] = 0 + (p_in[i][j] * 0.92 )- (p_out[i][j]/ 0.88)
                                else :
                                    e1[a] = e1[a-1] + (p_in[i][j] * 0.92 )- (p_out[i][j]/ 0.88)
                                a+=1
            ...
            storage_cost = ((171000/10)) * np.max(e1) #儲氫建置成本
        ...
        print(wind_c)
        Smin_cf  =str(round(min_cf*100,1)) 
        wb.save('C:\\Users\\farne\\Desktop\\專題\\code\\RFB24hrNR'+Smin_cf+'%min_cf.xlsx')
    min_cf+=0.05
